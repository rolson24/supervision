...
from supervision.tracker.byte_tracker.core import STrack, remove_duplicate_tracks
...
PREDICTIONS = np.array(
    [
        ...
        [299, 1225, 512, 1663, 0.73, 39, 44],
        [529, 874, 645, 945, 0.76, 39, 45],
...
LOW_CONFIDENCE_PREDICTIONS[:,4] = .1

# Test data value
DATA = {"number": [5*i for i in range(PREDICTIONS.shape[0])], "letter": ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i']}
...
# all low confidence predictions (<.1)
LOW_CONFIDENCE_TARGET_PREDICTIONS[:,4] = .1
...
def assert_strack_lists_equal(stracks, target_stracks):
    assert len(stracks) == len(target_stracks), f"Expected {len(target_stracks)} stracks, but got {len(stracks)}."
    for i in range(len(stracks)):
        ...
        assert strack.start_frame == target_strack.start_frame, f"Expected start frame {strack.start_frame}, but got {target_strack.start_frame}."
        assert strack.frame_id == target_strack.frame_id, f"Expected current frame {strack.frame_id}, but got {target_strack.frame_id}."


@pytest.mark.parametrize(
    "incoming_detections, target_detections, with_mask,"
    " exception",
    [
        (   # Test empty detections
            Detections.empty(),
            TARGET_EMPTY_DETECTIONS, # expect empty detections with tracker_id array
            False,
            DoesNotRaise(),
        ),
...
def test_update_with_detections(
    ...
    incoming_detections.xyxy = incoming_detections.xyxy.copy() if incoming_detections.xyxy is not None else None
    incoming_detections.confidence = incoming_detections.confidence.copy() if incoming_detections.confidence is not None else None
    incoming_detections.class_id = incoming_detections.class_id.copy() if incoming_detections.class_id is not None else None
    BYTE_TRACKER.reset()
    with exception:
        for i in range(6):
            incoming_detections.xyxy += 1
            if with_mask:
                mask = np.zeros((incoming_detections.xyxy.shape[0], IMG_HEIGHT, IMG_WIDTH))
                for i in range(incoming_detections.xyxy.shape[0]):
                    mask[i, # box
                        int(incoming_detections.xyxy[i,1]):int(incoming_detections.xyxy[i,3]), # y
                        int(incoming_detections.xyxy[i,0]):int(incoming_detections.xyxy[i,2])  # x
                        ] = 1
                incoming_detections.mask = mask
            tracked_detections = BYTE_TRACKER.update_with_detections(incoming_detections)
        
        assert (tracked_detections == target_detections)
    
TARGET_STRACKS = [STrack(tlwh=[det[0], det[1], det[2]-det[0], det[3]-det[1]], score=det[4], class_ids=det[5]) for det in TARGET_PREDICTIONS]
# TARGET_LOW_CONFIDENCE_STRACKS = [STrack(tlwh=detection[:4], score=detection[4], class_ids=detection[5]) for detection in LOW_CONFIDENCE_TARGET_PREDICTIONS]
